...
class FullHouse extends Rule {
  evalRoll = dice => {
    ...
    if (Object.keys(diceDict).length === 2 && (Object.values(diceDict).includes(2))){
      return this.score
    }
    else{
      return 0
...
class SmallStraight extends Rule {
  evalRoll = dice => {
    let d = new Set(dice); //6354
    d = Array.from(d); //array of 
    d.sort(); //3456
    let orderAns = 1;
    for (let i = 1; i < d.length; i++){
      ((d[i-1]+1 === d[i]) // 3+1 === 4, 4+1 === 5, 5+1 === 6
        ? orderAns +=1
        : orderAns = 0)
    }
    return (orderAns >= 4 
...
const fullHouse = new FullHouse({ score: 30 });
